...
- **作业全链路闭环**：教师发布作业 → 学生在线提交 → AI 自动批改 → 学情看板统计，作业管理和我的作业页面覆盖完整业务流程。
- **课程全生命周期管理**：教师创建/编辑/删除课程，学生选课/退课，课程成员角色分配（owner/teacher/assistant/student）。
- **知识库文档删除**：删除文档时同步清理数据库文本块并重建 FAISS 向量索引，避免检索到已删除内容。
- **多课程数据隔离**：课程、时间范围下拉变化后，看板/试题库自动按选中课程与时间重新拉取并刷新数据。
- **对话记录管理**：答疑会话支持删除（软删除进回收站）、从回收站恢复、彻底删除与批量操作。
- **试题回收站**：已删除试题可在回收站搜索、批量恢复或彻底删除，不再与答疑会话回收站混淆。
- **AI 出题自动入库**：出题 Agent 生成的题目写入题库，试题库页面按课程实时展示真实数据。
- **答题历史与正确率**：学生自测后可查看最近 50 条答题记录及正确率统计。
- **全角色页面真实数据接入**：教师/管理员看板、学生名单、热门问题、知识库、作业管理、作业批改、练习答题、系统管理、日志审计均接入后端真实 API，无 mock 占位数据。
- **多轮对话上下文**：SSE 流式答疑首帧回传 `session_id`，前端自动绑定会话，支持多轮上下文连续对话。
- **角色导航一致性**：管理员导航与教师共享教学管理入口（学情看板、学生名单、知识库、作业管理、作业批改、试题管理），额外拥有系统设置。
