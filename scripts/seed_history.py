"""回填 90 天历史演示数据，让学情看板的时间范围切换（7/30/90 天）有真实差异。

- 回填内容：答疑会话+消息（is_deleted=1，不进入答疑侧栏但计入看板）、
  历史作业+提交+批改、自测练习记录。
- 幂等：检测到已有超过 15 天前的答疑消息即跳过，可重复执行。
- 用法：.venv/Scripts/python.exe scripts/seed_history.py [course_id]   # 默认课程 1
"""
import random
import sys
...
def main() -> None:
    ...
    try:
        ...
        a1 = m.Assignment(course_id=course_id, title="第2章 线性表小测（历史演示）",
                          description="线性表与链表基础测验（历史演示数据）",
                          deadline=now - timedelta(days=75),
                          created_at=now - timedelta(days=80))
        a2 = m.Assignment(course_id=course_id, title="第3章 栈与队列小测（历史演示）",
                          description="栈与队列应用测验（历史演示数据）",
        ...
        db.commit()
        print(f"已回填课程 {course_id}：历史会话 {n_sessions}（侧栏不可见）、"
              f"消息 {n_messages}、提交 {n_sub}、批改 {n_graded}、练习记录 {n_lr}。")
    except Exception:
        db.rollback()
        raise
    finally:
