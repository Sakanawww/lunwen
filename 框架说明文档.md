...
├─────────────────────────────────────────────────────────┤
│                     API 路由层 (FastAPI)                  │
│  /chat | /kb | /grading | /questions | /dashboard ...  │
├─────────────────────────────────────────────────────────┤
│                    业务逻辑层 (Agents)                    │
│  TutorAgent | GraderAgent | QuestionAgent | LangGraph  │
├─────────────────────────────────────────────────────────┤
│                    服务层 (Services)                      │
│  LLM 封装 | Embedding | 知识库检索 | 会话管理            │
├─────────────────────────────────────────────────────────┤
│                    数据访问层 (Models)                    │
...
**扩展为 Redis**：修改 `_sessions` 为 Redis 客户端
...
**示例：修改聊天页**
1. 编辑 `templates/chat.html`
2. 调整样式 `static/css/chat.css`
3. 修改交互逻辑 `static/js/chat.js`
...
### Q3: 如何实现会话持久化？
**A**: 修改 `core/session.py`，将 `_sessions` 字典替换为 Redis 客户端。
...
| ADR-003 | 使用 Jinja2 而非 Vue/React | 毕业设计要求 + 简化部署 |
| ADR-004 | 内存态会话而非 Redis | 演示系统简化部署（可选降级） |
