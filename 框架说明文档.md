...
### 1.1 项目定位
这是一个**毕业设计演示系统**，实现基于多 Agent 协作的**智能课程助教与学情分析系统**，
涵盖答疑/批改/出题闭环 + 考勤管理/预警 + 平时表现评估/学情诊断 + 班级管理 + 课程公告。

### 1.2 核心特性
- **五 Agent 协作**：答疑 Agent、批改 Agent、出题 Agent、考勤预警 Agent、学情分析 Agent
- **RAG 知识库**：基于 FAISS 向量检索 + 引用溯源
- **LangGraph 编排**：有状态的多 Agent 工作流
- **Function Calling**：大模型自主工具调用
- **角色权限**：学生/教师/管理员三级 RBAC + **课程级 RBAC**（`require_course_role` 校验课程归属，教师只能访问自己课程的数据）
- **考勤管理**：考勤课次发起 + 学生签到 + 出勤率可视化 + AI 缺勤预警
- **平时表现评估**：四维平时分（出勤30% + 作业30% + 练习20% + 答疑20%）+ 四维得分分布 + 学生得分堆叠柱状图 + AI 学情诊断（独立二级页面）
- **二级路由页面**：学生详情、考勤明细、文档预览、练习答题、AI 诊断均为独立路由页面（替代 modal 弹窗）
- **Toast/ConfirmDialog**：全局通知组件替代浏览器原生 alert/confirm
- **班级管理**：班级实体 + 班级↔课程多对多关联 + 花名册
- **课程公告**：教师发布课程通知
...
│  AttendanceView | PerformanceView | AnnouncementsView    │
...
│   │
│   ├── agents/                   # 【核心】Agent 实现
│   │   ├── graph.py              # LangGraph 编排图（状态机，5 Agent 节点）
│   │   ├── tutor_agent.py        # 答疑 Agent（RAG+ 溯源）
│   │   ├── grader_agent.py       # 批改 Agent（评分 + 评语）
│   │   ├── question_agent.py     # 出题 Agent（试题生成）
│   │   ├── attendance_agent.py   # 考勤预警 Agent（缺勤提醒生成）
│   │   ├── performance_agent.py  # 学情分析 Agent（自然语言诊断）
│   │   └── orchestrator.py       # 编排器（5 Agent 关键词路由）
│   │
│   ├── api/                      # 【接口】RESTful API
│   │   ├── auth.py               # 认证/登录/注册
│   │   ├── chat.py               # 聊天会话接口
...
│   │   ├── grading.py            # 作业批改接口
│   │   ├── question.py           # 试题管理接口
│   │   ├── dashboard.py          # 仪表盘数据（含考勤+平时分摘要）
│   │   ├── practice.py           # 练习接口
│   │   ├── courses.py            # 课程管理
│   │   ├── classes.py            # 班级管理
│   │   ├── attendance.py         # 考勤管理 + 预警 Agent
│   │   ├── performance.py        # 平时表现评估 + 学情诊断 Agent
│   │   ├── announcements.py      # 课程公告
│   │   ├── accounts.py           # 账号管理
│   │   ├── logs.py               # 日志接口
...
class AgentState(TypedDict, total=False):
    ...
    result: dict           # 执行结果
    kb: object             # 知识库句柄
    absent_students: list  # 缺勤学生数据（考勤预警 Agent）
    perf_data: dict        # 学生平时分数据（学情分析 Agent）

# 路由逻辑
ROUTE_KEYWORDS = {
    "答疑Agent": ["答疑", "问题", "不会", "讲解", "概念", ...],
    "批改Agent": ["批改", "评分", "作业", "评语", ...],
    "出题Agent": ["出题", "试题", "测验", "题目", ...],
    "考勤预警Agent": ["缺勤", "旷课", "考勤", "出勤率", "预警", ...],
    "学情分析Agent": ["学情", "表现", "诊断", "平时分", "建议", ...],
}

# 图结构
router → 条件边 → 答疑/批改/出题/考勤预警/学情分析 Agent → END
```
